...
export type PackageJsonRepository = { type: "git"; url: string; directory?: string };
export type PackageJsonIssues = { url?: string; email?: string };
export type PackageJsonFuncding = { type?: "individual" | "patreon" | (string & {}); url?: string };
export type PackageJsonMeta = {
  name?: string;
  version?: string;
  description?: string;
  keywords?: string[];
  license?: License | "UNLICENSED" | `SEE LICENSE IN ${string}` | string;
  author?: PackageJsonPerson;
  contributors?: PackageJsonPerson[];
  homepage?: string;
  repository?: string | PackageJsonRepository;
  bugs?: string | PackageJsonIssues;
  funding?: string | PackageJsonFuncding | Array<string | PackageJsonFuncding>;
};
export type PackageJsonConditionalExport = Record<
  "node-addons" | "node" | "import" | "require" | "module-sync" | "react-native" | "default",
  string | Record<"types" | "default", string>
>;
export type PackageJsonModule = {
  type?: "module" | "commonjs";
  exports?: string | PackageJsonConditionalExport | Record<string, string | PackageJsonConditionalExport>;
  main?: string;
  types?: string;
  browser?: string;
};
export type PackageJsonBin = Record<string, string>;
export type PackageJsonFiles = {
  files?: string[];
  bin?: string | PackageJsonBin;
  man?: string | string[];
  directories?: { bin: string; man: string };
};
...
export type PackageJsonDependenciesRecord = Record<string, string>;
export type PackageJsonOverrides = { [member in "." | (string & {})]: string | PackageJsonOverrides };
export type PackageJsonDependencies = {
  dependencies?: PackageJsonDependenciesRecord;
  devDependencies?: PackageJsonDependenciesRecord;
  peerDependencies?: PackageJsonDependenciesRecord;
  peerDependenciesMeta?: Record<string, { optional?: boolean }>;
  bundleDependencies?: boolean | string[];
  optionalDependencies?: PackageJsonDependenciesRecord;
  overrides?: Record<string, string | PackageJsonOverrides>;
};
export type PackageJsonOS = "darwin" | "linux" | "win32" | "freebsd" | "openbsd" | "aix" | "sunos";
export type PackageJsonArch = "arm" | "arm64" | "ia32" | "loong64" | "mips" | "mipsel" | "ppc64" | "riscv64" | "s390" | "s390x" | "x64";
export type PackageJsonConstraints = {
  engines?: Record<"node" | "npm" | (string & {}), string>;
  os?: PackageJsonOS[] | `!${PackageJsonOS}`[];
