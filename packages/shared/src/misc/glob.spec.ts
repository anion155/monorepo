...
import { glob, globs, InvalidGlobPattern } from "./glob";
...
describe("glob module", () => {
  describe("glob()", () => {
    ...
    it("should match using wildcard **", () => {
      const values = ["file.txt", "test/nested.txt", "deep/nested/nested.txt", "image.png"];
      expect(values.filter(glob("*"))).toStrictEqual(["file.txt", "image.png"]);
      expect(values.filter(glob("**/*.txt"))).toStrictEqual(["file.txt", "test/nested.txt", "deep/nested/nested.txt"]);
    });
...
abcdefghijklmnop
  ...
  describe("globs()", () => {
    it("should match any glob", () => {
      expect(["file1.txt", "file2.txt", "file3.txt", "file4.txt"].filter(glob("file1*"))).toStrictEqual(["file1.txt"]);
      expect(["file1.txt", "file2.txt", "file3.txt", "file4.txt"].filter(glob("file*2.txt"))).toStrictEqual(["file2.txt"]);
      expect(["file1.txt", "file2.txt", "file3.txt", "file4.txt"].filter(globs(["file1*", "file*2.txt"]))).toStrictEqual(["file1.txt", "file2.txt"]);
      expect(["file1.txt", "file2.txt", "file3.txt", "file4.txt"].filter(globs(["file*", "!*3*"]))).toStrictEqual([
        "file1.txt",
        "file2.txt",
        "file4.txt",
      ]);
      expect(["src/index.js", "src/index.test.js", "src/test.js", "src/test.test.js"].filter(globs(["src/*.js", "!src/*.test.js"]))).toStrictEqual([
        "src/index.js",
        "src/test.js",
      ]);
      expect(
        ["src/index.js", "src/index.test.js", "src/nested/module.js", "src/nested/module.test.js", "src/test.js", "src/test.test.js"].filter(
          globs(["src/*.js", "!src/**/*.test.js"]),
        ),
      ).toStrictEqual(["src/index.js", "src/test.js"]);
      expect(
        ["src/index.js", "src/index.test.js", "src/nested/module.js", "src/nested/module.test.js", "src/test.js", "src/test.test.js"].filter(
          globs(["src/**/*.js", "!src/**/*.test.js"]),
        ),
      ).toStrictEqual(["src/index.js", "src/nested/module.js", "src/test.js"]);
    });
  });
});
